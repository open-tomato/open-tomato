# REFACTOR_NEEDED — @open-tomato/cache
...
- [ ] Depends on runtime-split build (`dist/browser.js`, `dist/server.js`); verify `exports` map + `files` whitelist + `publint` pass.
- [ ] No README covering browser/server split.
- [ ] `peerDependencies` on `ioredis` — confirm version range matches consumer expectations.
