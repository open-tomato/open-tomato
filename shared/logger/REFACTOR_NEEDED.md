# REFACTOR_NEEDED — @open-tomato/logger
...
- [ ] Depends on runtime-split build (`dist/node.js`, `dist/browser.js`); verify `exports` map + `files` whitelist + `publint` pass.
- [ ] README needs to cover browser/node selection behavior.
