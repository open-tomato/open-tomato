...
- [ ] Create `packages/shared/platform-core/package.json` with name `@open-tomato/platform-core`, version `0.1.0`, `private: false`, `publishConfig.access: public`, `type: module`, `main: ./src/index.ts`, `exports: { ".": "./src/index.ts" }`, devDependencies on `@open-tomato/eslint-config` and `@open-tomato/typescript-config` (both `workspace:^`), `eslint`, `typescript`, `vitest`, `@types/node`
- [ ] Create `packages/shared/platform-core/tsconfig.json` extending `@open-tomato/typescript-config/base.json` with `include: ["src/**/*"]`
- [ ] Create `packages/shared/platform-core/eslint.config.mjs` re-exporting `@open-tomato/eslint-config`
- [ ] Create `packages/shared/platform-core/README.md` documenting the `PlatformPlugin` contract and giving an example plugin skeleton
