...
- [ ] Write `src/provision.ts` exporting `ProvisionSchema` Zod schema as a discriminated union of `z.literal(true)`, `z.literal(false)`, `z.object({ ... })` with optional fields `disabled`, `caps`, `metadata`
