...
- [ ] Read [`AGENTS.md`](./AGENTS.md), [`NEXT-ITERATIONS.md`](./NEXT-ITERATIONS.md), and [`skills/atom-authoring/SKILL.md`](./skills/atom-authoring/SKILL.md) end-to-end before any code change
- [ ] Confirm phase 1 baseline: `bun install && bun run check-types && bun run test && bun run build && bun run build-storybook` all exit 0 from [`packages/ui-skeleton/`](./)
...
- [ ] Refactor [`src/atoms/Progress/`](./src/atoms/Progress/) per A1 recipe
- [ ] Refactor [`src/atoms/Card/`](./src/atoms/Card/) per A1 recipe (multi-part internally, but no per-part className exposed)

```text
# Stage: Atom className refactor — wrapper-frame atoms (A2 recipe)
```

- [ ] Rewrite [`src/atoms/Input/input.variants.ts`](./src/atoms/Input/input.variants.ts) to consume `wrapperFrameVariants` with added `density` + `tone` axes
...
- [ ] Repeat A2 sequence for [`src/atoms/Textarea/`](./src/atoms/Textarea/), substituting `min-h-*` for `h-*` in density-compact branch
...
- [ ] Add `frame` axis to [`src/atoms/ScrollArea/scroll-area.variants.ts`](./src/atoms/ScrollArea/scroll-area.variants.ts) and add `scrollAreaViewportVariants` with `padding` axis
...
- [ ] `bun run build` — confirm `dist/molecules/<Each>.js` files emitted, `dist/molecules/<Each>/index.d.ts` declarations preserved, `dist/molecules.js` layer barrel exists
