...
Pagination is the canonical demonstration of the **exported pure helper at the top of the component file** pattern. The range computation (`buildPaginationRange({ page, pageCount, siblingCount })`) sits above the component, takes a plain props bag, and returns a discriminated-union array of `{ type: 'page', page } | { type: 'ellipsis', position }` descriptors. Exporting the helper alongside the component triggers `react-refresh/only-export-components` (the rule warns on any non-component value export in a file that also exports a component — type exports are fine, function exports are not). Apply a focused `// eslint-disable-next-line react-refresh/only-export-components` with a justification comment on the function declaration when the helper is genuinely useful to adjacent UI (the trade-off is HMR-only, dev-time only). The precedent for focused eslint-disable comments lives in `src/index.ts`. Tests can validate the helper directly without rendering — pure helpers are the cheapest test surface in the organism layer.

InputGroup is the canonical demonstration of the **compose-atom-primitives-not-the-atom-component** escape valve. When an organism owns an atom-equivalent surface (a single bordered frame with `focus-within` ring + size-aware padding) AND adds slots outside the inner control, rendering the atom component inside produces a doubled frame and a doubled `focus-within:ring-*` on the inner wrapper. The fix is to import the atom's variant primitives directly: InputGroup imports `inputControlVariants` from `@/atoms/Input` and `wrapperFrameVariants` from `@/particles/wrapper-frame.variants`, then applies them on a native `<input>` (for the inner transparent control) and the organism's own outer `<div>` (for the shared border + focus-within ring). This still counts as composition — the atom's primitives drive the visual contract and the data-slot vocabulary stays consistent — it just avoids the visual double-up. The discriminator for "render the atom component vs import its variant primitives" is whether the organism *wraps* the atom (Field wraps Input — atom component is fine) or *replaces* the atom's surface with a slot-extended version (InputGroup replaces Input's surface — import the primitives). The same rule applies to any future organism that owns a wrapper-frame-shaped surface (a hypothetical TextareaGroup, SelectGroup, etc.).
