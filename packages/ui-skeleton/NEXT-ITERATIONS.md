...
**Re-verified 2026-05-24:** still warning, no functional impact. `bun install` completes cleanly and `bun run build` succeeds; not chasing minor-version Storybook bumps speculatively.
...
**Re-verified 2026-05-24:** still warning, no functional impact. `bun run build` emits the documented per-entry + hashed shared-chunk layout; no consumer issue has surfaced.
