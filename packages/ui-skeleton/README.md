...
- **Phase 2 audit (2026):** the taxonomy below was audited end-to-end during phase 2. **Calendar** and **Combobox** were reclassified from Molecules to Organisms — Calendar owns date-grid state, Combobox composes the Popover molecule, and both violate the rule that molecules must not compose other molecules. The "Somewhat between Molecules and Organisms" allowance above no longer permits net-new molecule-composes-molecule wrappers: layer-import direction is enforced by ESLint (`no-restricted-imports`) under `src/molecules/**` and `src/atoms/**`. Likewise, the `className` / inline-style "avoided as much as possible" guidance is now a hard rule for atoms and molecules — public `className` is omitted from props types, no consumer string is forwarded through `cn(...)`, and per-part bag props that proxy to inner sub-components omit `className` too. See `AGENTS.md` cardinal rules #1 and #11 for the enforcement details.
- **Phase 3 audit (2026-05-23):** the organism layer is being authored under the same six-file convention used for atoms and molecules. Organisms MAY compose `@/molecules/*` + `@/atoms/*` + `@/particles/*` but MUST NOT import `@/organisms/*` or any upward layer — when an organism would compose another organism, promote it to a template instead. Cardinal rule #1 (no public `className`) now extends to organisms, and cardinal rule #11 (layer-import direction) gains an organism row; both are enforced by ESLint (`no-restricted-imports`) under `src/organisms/**`. Accordion is the canonical organism reference; see `skills/organism-authoring/SKILL.md` and `AGENTS.md` cardinal rules #1 and #11 for the enforcement details.
- **Phase 3 close-out audit (2026-05-23):** the organism layer is complete. All 17 organisms listed below — Accordion, AlertDialog, Breadcrumb, Calendar, Combobox, Command, DataTable, Dialog, Drawer, DropdownMenu, Empty, Field, InputGroup, Menubar, Pagination, Resizable, and Sonner — ship under `src/organisms/` with the six-file convention, per-entry Vite output under `dist/organisms/`, registry manifest coverage, and Storybook autodocs. Date Picker was held back from the organism layer and will be promoted to a template in Phase 4: it composes the Popover molecule and the Calendar organism, which would force an organism-composes-organism import that the layer-import guard blocks. See `skills/organism-authoring/SKILL.md` for the promote-to-template rule.
- **Phase 4 audit (2026-05-23):** the template layer is being authored under the same six-file convention used for atoms, molecules, and organisms. Templates MAY compose `@/organisms/*` + `@/molecules/*` + `@/atoms/*` + `@/particles/*` but MUST NOT import `@/templates/*`, `@/pages/*`, or `@/providers/*` — enforced by ESLint (`no-restricted-imports`) under `src/templates/**`. The README's use of "Templates" here is loose: NavigationMenu, Sheet, Sidebar, Tabs, and the promoted Date Picker frame the page surface as composable primitives rather than full page skeletons, which deviates from textbook atomic design — the deviation is sanctioned at this layer. Public `className` remains blocked across the template layer; layout-level granularity flows through variant axes only. Tabs is the canonical template reference; Date Picker moves from Organisms to Templates in the inventory below. See `skills/template-authoring/SKILL.md` and `AGENTS.md` cardinal rules #1 and #11 for the enforcement details.
- **Phase 4 close-out audit (2026-05-24):** the template layer is complete. All five templates listed below — DatePicker, NavigationMenu, Sheet, Sidebar, and Tabs — ship under `src/templates/` with the six-file convention, per-entry Vite output under `dist/templates/`, registry manifest coverage, and Storybook autodocs. Sheet composes the Dialog organism with an inline `style` override + `data-side` attribute escape pattern to anchor the surface to each cardinal edge (template-composes-organism is allowed at this layer; the inline-style escape is sanctioned because Dialog's `contentProps` intentionally omits `'className'`). Sidebar is self-contained (no Sheet composition — the shared anchored-surface treatment was left as a future particle if a second consumer ever appears) and DatePicker composes Popover (molecule) + Calendar (organism) via the controlled-passthrough pattern documented in `skills/template-authoring/SKILL.md`.
- **Phase 5 audit (2026-05-24):** the provider layer is being authored under a sanctioned collapse of the six-file convention — `<Provider>.tsx`, `<Provider>.test.tsx`, optional `<Provider>.stories.tsx`, `README.md`, and `index.ts`; the `<provider>.variants.ts` file is omitted because providers expose context rather than visual output (no rendered DOM beyond `children`, no `className` surface, no CVA axes). Providers MAY import only `@/particles/*` and MUST NOT import `@/atoms/*`, `@/molecules/*`, `@/organisms/*`, `@/templates/*`, `@/pages/*`, or other `@/providers/*` — enforced by ESLint (`no-restricted-imports`) under `src/providers/**`. Direction is the canonical provider reference. See `skills/provider-authoring/SKILL.md` and `AGENTS.md` cardinal rule #11 for the enforcement details and the file-shape collapse conditions.
- **Phase 5 close-out audit (2026-05-24):** the provider layer is complete. The single provider listed below — Direction — ships under `src/providers/` with the sanctioned four-file shape (`Direction.tsx`, `Direction.test.tsx`, `README.md`, `index.ts`; both the `direction.variants.ts` and `Direction.stories.tsx` files are intentionally absent per the file-shape collapse documented in the Phase 5 audit above), per-entry Vite output under `dist/providers/Direction.js`, and registry manifest coverage. The Storybook static manifest at `/index.json` contains no Direction entry — also intentional, since stories don't apply to a context-only provider. End-of-phase gates (`bun run check-types && bun run test && bun run lint && bun run build && bun run build-storybook`) all pass.
