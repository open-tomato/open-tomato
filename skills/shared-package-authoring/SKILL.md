...
| Stub function parameter (`_arg: T`) trips `@typescript-eslint/no-unused-vars` | `@open-tomato/eslint-config/base` does not set `argsIgnorePattern: '^_'`, so the underscore convention is **not** honored | Reference the param in the body — `void arg;` for a no-op, or (better) interpolate the value into the stub's `throw new Error(...)`. Underscore prefix alone won't silence the lint. |
