...
- [ ] Write `cli/src/discovery/findRoot.ts` exporting `findOpenTomatoRoot(startDir: string): string | null` that walks parent directories looking for `.open-tomato-root`
- [ ] Stop the walk at the filesystem root (return null) rather than crossing the boundary
- [ ] Return the directory containing the marker (not the marker path itself)
